...
| AIPersona-{env} | VPC + DynamoDB + ECS Express Service | AIPersonaEcr |
...
### AIPersona-{env}
- `ServiceEndpoint`: アプリケーションURL（AWS提供ドメイン）
...
1. EC2 > Load Balancers でExpress Modeが作成したALBを確認
2. リスナールールにCognito認証アクションを追加
3. 出力された`UserPoolId`、`UserPoolClientId`を使用
...
1. CallbackUrlがサービスエンドポイントと一致しているか確認
