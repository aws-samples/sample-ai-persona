...
export class ExpressService extends Construct {
  ...
  constructor(scope: Construct, id: string, props: ExpressServiceProps) {
    ...
    const {
      ...
      bedrockModelId, agentModelId, enableAdditionalPersonaModels = false,
    ...
    if (enableAdditionalPersonaModels) {
      // 追加ペルソナベースモデル（GPT-5.6 Terra/Luna、Gemma 4 31B。いずれもBedrock Mantle
      // 経由で呼び出す非Anthropicモデル）のための権限。
