...
例外の送出から画面表示までの規約（`ErrorCode` / `ErrorKind` / `CodedError`、各層の持ち物、
`kind` による表示分岐、htmx 制約、`toast_response()` / `mark_renderable()`、永続化失敗の扱い、
エラー表示テンプレート、422 / `HTTPException` のグローバルハンドラ、機械検査）は
**`.claude/rules/error-catalog.md` を単一ソースとする。** ここには複製しない。
