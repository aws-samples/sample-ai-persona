...
- [8. システム設定](#8-システム設定)
  - [8.1 データ連携機能（MCPサーバー）](#81-データ連携機能mcpサーバー)
...
- 通常ファイルは全文をテキスト抽出してAIに渡しますが、CSVは先頭20行のみをプレビューとして渡し、残りの全データはAIエージェントがSQLクエリツールで必要に応じて参照します
- そのため、生成されたペルソナが21行目以降のデータ（プレビュー範囲外）の傾向を反映しているかは、「生成ログ・評価」で実行されたクエリ（tool_call/tool_result）を確認することで検証できます（[2. ペルソナ管理](#2-ペルソナ管理)参照）
...
- 「データ連携機能」セクションのチェックボックスで、外部データセット連携用のMCPサーバーをON/OFFできます
- ON時は「MCPサーバーが起動中です」、OFF時は停止中である旨がステータス表示されます
