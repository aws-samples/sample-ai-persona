...
1. メインメニューから「ペルソナ生成」→「インタビューデータ」タブを選択
2. テキストファイル（`.txt`, `.md`）をアップロード（10MB以下）
3. 「アップロード」ボタンをクリック（生成に1〜2分）
...
1. メインメニューから「ペルソナ生成」→「調査・分析レポート」タブを選択
...
メインメニューから「ペルソナ管理」を選択すると、保存済みペルソナの一覧が表示されます。
...
- **手動追加**: 「知識を追加」→ トピック名と内容を入力 → 「追加」
- **ファイルから追加**: 「ファイルから知識を追加」→ ファイルをアップロード
  - 対応形式: PDF, Word, PowerPoint, Excel, テキスト（10MB以下）
...
ペルソナ詳細画面の「ナレッジベース」セクションで、Amazon Bedrock Knowledge Baseをペルソナに紐付けできます。紐付けたナレッジベースの情報は議論・インタビュー時にペルソナが参照します。ナレッジベースの作成・管理は「システム設定」ページから行います。

---

## 3. 議論・インタビューの実行

メインメニューから「議論設定」を選択して開始します。
...
議論結果ページから「議論履歴」を選択すると、保存済み議論の一覧が表示されます。

- 日付、参加ペルソナ、トピックで検索・フィルタリング可能
- 議論モード（従来/AIエージェント/インタビュー）がバッジで表示
...
メインメニューから「アンケート」→「ペルソナデータ設定」を選択します。
...
メインメニューから「アンケート」→「テンプレート管理」を選択します。
...
1. 「アンケート開始」メニューを選択
2. **データソース選択**: Nemotronまたはカスタムデータセット
3. **テンプレート選択**: 保存済みテンプレートから選択
4. **設定入力**: アンケート名、説明（オプション）、対象ペルソナ数（100〜10000）
5. **属性フィルタ（オプション）**: 性別、年齢、職業、出身国、居住地域、都道府県、結婚・子供の有無
   - 「ペルソナ統計プレビュー」で対象ペルソナの属性分布を事前確認可能
6. 「アンケートを開始」をクリック → バックグラウンドで実行（画面を閉じてもOK）
...
> - 「その他項目」に補足情報を付けると、ペルソナの回答がより豊かになります
...
3〜5ラウンドを推奨します。1ラウンドでは浅い議論になり、10ラウンドでは時間がかかりすぎる可能性があります。
</details>
...
しっかり議論モードまたはインタビューモードで「長期記憶を有効にする」チェックボックスをオンにしてください。システム管理者がAgentCore Memoryを設定している必要があります。
</details>

<details>
<summary>Q: 手動でペルソナに知識を追加できる？</summary>

はい。ペルソナ詳細画面の「知識・事実」タブから「知識を追加」ボタンで、トピック名と内容を入力して保存できます。
</details>
